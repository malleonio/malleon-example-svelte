# Malleon Replay SDK - Svelte Example

This is a complete Svelte example application demonstrating how to integrate the **Malleon Replay SDK** into your Svelte application. The example shows how to capture user interactions, console logs, API calls, and custom metadata.
...
5. **Open your browser**
   ...
   Navigate to `http://localhost:4203` to see the example application.
...
```
malleon-example-svelte/
├── src/
│   ├── App.svelte              # Main component with demo buttons
...
Import the `replayService` in your Svelte components and use it to:
...
- Type-safe integration with Svelte
...
- `--base-url`: The full base URL where your app is served (e.g., `http://localhost:4203`, `https://myapp.com`)
- `--release`: Optional release version (e.g., `1.0.0`)
- `--source-map-dir`: Path to your built source maps directory (default: `dist`)

For Svelte apps built with Vite, the source maps are typically in `dist` after building.
...
- **Wrong file paths in stack traces?**
  ...
  - For local development: `--host localhost --base-url http://localhost:4203`
